...
def main():
  ...
  <div style="position:relative;height:720px;overflow:hidden;background:#0B0B0C">
    ...
    gallery("Бобби Джонс · клюшка целиком", "Клюшки · 1 из 6",
            [img("src/jones_04"), img("src/jones_00"), img("src/jones_09"),
             img("src/jones_02"), img("src/jones_01"), img("jones_sig")],
            3, 200,
            "Оригинальные кадры лота: клюшка в полную длину, головка с латунной лентой и клеймом "
            "мастера, подошва. Ниже — подписанная фотография, которая идёт вместе с клюшкой, "
            "она же рядом с самой клюшкой, и росчерк «Bob Jones» крупно.")
  ...
  <div style="height:720px;overflow:hidden"><img src="{img('framed_flag22')}" alt="" style="width:100%;height:100%;object-fit:cover"></div>
  <div style="display:flex;flex-direction:column;justify-content:center;padding:56px 58px 56px 40px">
    ...
    цвет Огасты и пиджака победителя. Флаг растягивается под стеклом на паспарту, тонкая
    золотая линия по периметру, латунная табличка внизу. Жёлтое полотно на зелёном узнают
    без единой подписи.</p>
