#!/usr/bin/env python3
"""«Славные парни» и круг Скорсезе — горизонтальная дека.

    python3 tools_kat_build.py H    # → deck_mar_H.html  1280×720
    python3 tools_kat_build.py V    # → deck_mar_V.html  720×1280
...
from tools_mar_data import LOTS, ORDER, CHAPTERS
...
NAMES_MAP = {"duo": "Пеши и Лиотта", "keitel": "Кейтель",
             "dir": "Режиссёры", "cast": "Та же компания"}
...
def main():
  ...
  <div style="position:relative;background:#0B0B0C">
    ...
    <div style="position:absolute;inset:0;display:flex;flex-direction:column;justify-content:flex-end;padding:0 48px 40px">
      ...
      <h1 class="serif" style="font-weight:300;font-size:66px;line-height:1.03;color:var(--ivory)">«Славные парни»</h1>
      <p style="font:300 22px/1.5 'Inter';color:#b8b2a6;margin-top:20px">Четырнадцать предметов: фотографии, постер
      и полный сценарий.</p>
    </div>
  </div>
  ...
  <div style="display:flex;flex-direction:column;justify-content:space-between;padding:56px 52px">
    ...
    <div>
      ...
      <h1 class="serif" style="font-weight:300;font-size:62px;line-height:1.04;color:var(--ivory)">«Славные парни»<br>и круг Скорсезе</h1>
      <p style="font:300 18px/1.6 'Inter';color:#b8b2a6;margin-top:22px;max-width:42ch">Четырнадцать предметов: фотографии, постер «Таксиста»
      и полный сценарий «Бешеных псов». Актёры, режиссёры и второй план — один круг американского кино.</p>
    </div>
  ...
  <div style="position:relative;background:#fff;display:flex;align-items:center;justify-content:center;padding:56px 46px">
    ...
    # ---------- главы
    HERO = {"duo": img("liotta_fod"), "keitel": img("keitel_taxi"),
            "dir": img("coppola_jsa"), "cast": img("darrow")}
    NAMES = NAMES_MAP
  ...
  <div style="padding:0 {'44px' if V else '66px'};display:flex;flex-direction:column;justify-content:center">
    ...
    <h2 class="serif" style="font-weight:300;font-size:{'50px' if V else '44px'};line-height:1.12;color:var(--ivory);margin-top:34px;max-width:{'100%' if V else '32ch'}">«Славные парни» и круг Скорсезе. Четырнадцать предметов, один круг кино.</h2>
  </div>
...
<title>Stargift · Славные парни</title>
...
"""
    open(f"{R}/deck_mar_{tag}.html", "w", encoding="utf-8").write(doc)
    print(f"deck_mar_{tag}.html: {n} слайдов, {W}×{H}")
